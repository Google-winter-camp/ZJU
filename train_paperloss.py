...
class Adptive_loss(nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        image_label = self.convert_one_hot(labels, L)  # 512,340
        ...
        #########
        vi = torch.matmul(image_label, self.word_embedding)  # 512,300
        ...
        zeros = torch.zeros(512, 340)
        ######

        loss1 = margin_parm + cos_B - 2 * cos_A.unsqueeze(1)
        final_loss = torch.sum(torch.max(zeros, loss1))
